...
void p3GRouter::handleLowLevelTransactionAckItem(RsGRouterTransactionAcknItem *trans_ack_item)
{
    ...
    {
        RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): Received Transaction ACK for message " << std::hex << trans_ack_item->propagation_id << std::dec << " via friend/tunnel " << trans_ack_item->PeerId() << ". Setting status to SENT.";
        it->second.data_status = RS_GROUTER_DATA_STATUS_SENT;
        it->second.last_sent_TS = time(NULL) ;
    }
    else
        RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): Received Transaction ACK but no corresponding pending message found (id=" << std::hex << trans_ack_item->propagation_id << std::dec << ")";
}
...
#endif
    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter Tunnel - Discovered new virtual peer (path) " << std::hex << virtual_peer_id << std::dec << " for hash " << hash << ". Resetting stabilisation timer.";
...
void p3GRouter::handleTunnels()
{
    ...
    for(std::map<GRouterMsgPropagationId, GRouterRoutingInfo>::iterator it=_pending_messages.begin();it!=_pending_messages.end();++it)
    {
        ...
        if(it->second.data_status == RS_GROUTER_DATA_STATUS_PENDING && (it->second.routing_flags & GRouterRoutingInfo::ROUTING_FLAGS_ALLOW_TUNNELS))
        {
            ...
            {
                RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Turtle search timeout (" << MAX_TUNNEL_WAIT_TIME << "s) reached for GRouter Msg ID " << std::hex << it->first << std::dec << ". Stopping search and entering " << MAX_TUNNEL_UNMANAGED_TIME << "s UNMANAGED cooldown.";
...
#endif
        RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Triggering Turtle search (monitorTunnels) for GRouter Msg ID " << std::hex << priority_list[i].second->data_item->routing_id << std::dec << " with deterministic SHA-1 hash " << priority_list[i].second->tunnel_hash;
...
void p3GRouter::routePendingObjects()
{
    ...
    for(std::map<GRouterMsgPropagationId, GRouterRoutingInfo>::iterator it=_pending_messages.begin();it!=_pending_messages.end();++it)
    {
	    ...
	    {
		RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter Tunnel - Attempting to send GRouter Msg ID " << std::hex << it->first << std::dec
            ...
            if(!peers_and_duplication_factors.empty())
	    {
		RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter Matrix - Attempting to send GRouter Msg ID " << std::hex << it->first << std::dec
		        << " via " << peers_and_duplication_factors.size()
            ...
            else
            {
                ...
                {
                    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Direct friend try delay expired for GRouter Msg ID " << std::hex << it->first << std::dec
        ...
        else if(it->second.data_status == RS_GROUTER_DATA_STATUS_ONGOING && now > MAX_TRANSACTION_ACK_WAITING_TIME + it->second.data_transaction_TS)
        {
            ...
            if( (it->second.routing_flags & GRouterRoutingInfo::ROUTING_FLAGS_IS_ORIGIN) && it->second.last_sent_TS + MAX_DELAY_FOR_RESEND < now)
            {
                if (it->second.data_status != RS_GROUTER_DATA_STATUS_PENDING)
                {
                    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Resend delay (" << MAX_DELAY_FOR_RESEND << "s) reached for GRouter Msg ID " << std::hex << it->first << std::dec
            ...
            else
            {
                if (it->second.routing_flags & GRouterRoutingInfo::ROUTING_FLAGS_ALLOW_TUNNELS)
                {
                    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - GRouter Msg ID " << std::hex << it->first << std::dec
...
#endif
        RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter Tunnel - Found active paths but WAITING for stabilisation delay (" << TUNNEL_OK_WAIT_TIME << "s) before sending data over hash " << hash;
        return ;
    }
...
bool p3GRouter::locked_sendTransactionData(const RsPeerId& pid,const RsGRouterTransactionItem& trans_item)
{
    if(mTurtle->isTurtlePeer(pid))
    {
        ...
        {
            const RsGRouterTransactionChunkItem* chunk = dynamic_cast<const RsGRouterTransactionChunkItem*>(&trans_item);
            if (chunk)
            {
                RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter Tunnel - Sending transaction chunk for GRouter Msg ID " << std::hex << chunk->propagation_id << std::dec
        ...
        {
            const RsGRouterTransactionAcknItem* ack = dynamic_cast<const RsGRouterTransactionAcknItem*>(&trans_item);
            if (ack)
            {
                RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter Tunnel - Sending transaction ACK for GRouter Msg ID " << std::hex << ack->propagation_id << std::dec
                        << " via Turtle tunnel to virtual PeerId " << std::hex << pid << std::dec;
            }
        }
#ifdef GROUTER_DEBUG
    ...
    else
    {
        if (trans_item.PacketSubType() == RS_PKT_SUBTYPE_GROUTER_TRANSACTION_CHUNK)
        {
            ...
            if (chunk)
            {
                RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter Matrix - Sending transaction chunk for GRouter Msg ID " << std::hex << chunk->propagation_id << std::dec
        ...
        else if (trans_item.PacketSubType() == RS_PKT_SUBTYPE_GROUTER_TRANSACTION_ACKN)
        {
            ...
            if (ack)
            {
                RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter Matrix - Sending transaction ACK for GRouter Msg ID " << std::hex << ack->propagation_id << std::dec
                        << " to direct friend PeerId " << std::hex << pid << std::dec;
            }
        }
#ifdef GROUTER_DEBUG
...

    {
        ...
        for(std::map<GRouterMsgPropagationId, GRouterRoutingInfo>::iterator it=_pending_messages.begin();it!=_pending_messages.end();)
        {
            ...
            if(delete_entry)
            {
                ...
                {
                    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Message GRouter Msg ID " << std::hex << it->first << std::dec
                            << " expired in GRouter cache after 48h (status was " << (int)it->second.data_status
                            << "). Discarding message.";
                }
                else
                {
                    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Cleaning fully delivered/done message GRouter Msg ID " << std::hex << it->first << std::dec
                            << " from cache.";
                }
#ifdef GROUTER_DEBUG
...
{
    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Received signed receipt from peer " << receipt_item->PeerId()
            << " for GRouter Msg ID " << std::hex << receipt_item->routing_id << std::dec
            << " destined to " << receipt_item->destination_key;

    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): handleIncomingReceiptItem() from peer " << receipt_item->PeerId();
    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "):   routing_id : " << std::hex << receipt_item->routing_id << std::dec;
    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "):   destination: " << receipt_item->destination_key;
    bool changed = false ;
#ifdef GROUTER_DEBUG
...
{
    RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Received data item from PeerId " << std::hex << data_item->PeerId() << std::dec
    ...
    {
        RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Message GRouter Msg ID " << std::hex << data_item->routing_id << std::dec
    ...
    if(clientService && !item_is_already_known)
    {
        ...
        {
            RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Decryption failed for message GRouter Msg ID " << std::hex << data_item->routing_id << std::dec;
            std::cerr << "    decrypting item : FAILED! Item cannot be passed to the client." << std::endl;
            delete decrypted_item ;

            return ;
        }
        RsDbg() << "MAIL (" << rsPeers->getPeerName(rsPeers->getOwnId()) << "): GRouter - Successfully decrypted message GRouter Msg ID " << std::hex << data_item->routing_id << std::dec
